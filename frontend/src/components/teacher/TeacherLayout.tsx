...
const navItems = [
  ...
  { to: '/teacher/classrooms', icon: Users, label: 'Classrooms' },
];
...
export const TeacherLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const initials =
    [user?.firstName?.[0], user?.lastName?.[0]].filter(Boolean).join('') || 'T';

  return (
    <div className="min-h-screen bg-ink-950 text-bone-100 font-sans">
      <div className="flex">
        <aside className="fixed inset-y-0 left-0 w-60 border-r border-emerald-500/[0.08] bg-ink-950 z-40 flex flex-col">
          <div className="px-5 py-5 border-b border-white/[0.06]">
            <Link to="/teacher" className="inline-flex items-center gap-2.5 group">
              ...
              <div>
                ...
                <div className="text-[10px] text-emerald-400 uppercase tracking-wider mt-0.5 font-medium">
                  Teacher portal
                </div>
              </div>
            </Link>
          ...
          <div className="px-3 py-3 border-t border-white/[0.06]">
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/[0.02]">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-emerald-400 truncate uppercase tracking-wider font-medium">
                  Teacher
                </div>
              </div>
              <button
